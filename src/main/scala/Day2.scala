...
val input = scala.io.Source.fromFile("AdventOfCodeDay2Input.txt").getLines

input.map(s => move(Move.valueOf(s.substring(0, 1)), Move.valueOf(s.substring(2, 3)))).sum
...
input
  .map(s => (Move.valueOf(s.substring(0, 1)), Result.valueOf(s.substring(2, 3))))
